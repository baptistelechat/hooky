...
| `PreToolUse`         | `working` / `searching` / `listening` | `listening` si `tool_name` ∈ `AskUserQuestion, ExitPlanMode` (l'outil EST l'attente de ta réponse : `PostToolUse` ne part qu'à ta réponse, et `working` n'a aucun timeout) ; `searching` si `tool_name` ∈ `Grep, WebSearch, Glob, WebFetch` ; sinon `working` | `waiting` / `running / review` |
...
| `complete`                          | `Stop`                                                                                                                                                                                           |
| `mention`                           | `permission_prompt`, `agent_needs_input`, `elicitation_dialog`, `elicitation_url_dialog`, `quota_auto_resume_disabled`, `PreToolUse` d'un outil qui t'attend (`AskUserQuestion`, `ExitPlanMode`) |
