...
- [ZBLK-038](archive/blockers/ZBLK-038.md) — Vite orphelin sur 1420 masquait la nouvelle config

## 2026-09-25

Baptiste abandonne les orbs shadercn « pour le moment » et demande de tout retirer avant de clôturer. Premier passage de ma part : `git revert --no-commit 9fde588`, qui a bien donné un retour propre (lint et build verts, changelog inchangé puisque le spike n'avait jamais été livré). Puis le `git commit` de `/session-close` est sorti vide : la session « Personnalisation de l'installateur Hooky », active dans le même dossier, avait lancé un `git reset` qui a effacé mon staging et ramené les orbs (`git reflog` + `list_sessions` pour le prouver, [ZBLK-039](archive/blockers/ZBLK-039.md), [LRN-106](learnings/LRN-106.md)). Je me suis arrêté et j'ai proposé deux options plutôt que de refaire en aveugle. Baptiste a alors précisé qu'il ne voulait **pas** de revert de commit mais un nettoyage manuel du code, réécrire l'historique étant destructif pour le projet ([BDR-094](decisions/BDR-094.md)).

Refait à la main : `Avatar.tsx` remis à l'identique (branche `SPIKE_ORB` et import `Orb01` retirés), plugin `typegpu` retiré de `vite.config.ts`/`.js`, dossier `src/components/orbs/` supprimé par `git rm -r` (le `rm -rf` combiné avait été refusé), quatre dépendances retirées par `pnpm remove`. Deux bruits neutralisés avant le commit : le hook formateur qui recompactait un import de `Avatar.tsx`, et `vite.config.js` que `pnpm build` réécrit à chaque fois ([LRN-108](learnings/LRN-108.md)). Preuve finale : `git diff 884a9d2 --stat` vide sur toutes les cibles ([LRN-107](learnings/LRN-107.md)). Commit `47b7f65` (🔥), sans push, en ne stageant que mes fichiers : `src-tauri/tauri.conf.json` et `src-tauri/installer/` sont restés à l'autre session. Choix de Baptiste pour la mémoire : tout en local, rien en global. `BLK-038` archivé en `ZBLK-038`, `BDR-093` passé en `révisé`. La question de la licence des shaders (piste XorDev, shaders maison, orbs installés par l'utilisateur) reste ouverte mais non engagée.

**Entrées clés :**

- [BDR-094](decisions/BDR-094.md) — orbs abandonnés, retirés à la main (sans revert)
- [ZBLK-039](archive/blockers/ZBLK-039.md) — revert stagé effacé par le `git reset` d'une autre session

---

Personnalisation de l'installateur NSIS de Hooky, en parallèle de la session qui retirait les orbs shadercn. Point de départ : la capture d'un téléchargement Windows montrant l'icône générique de NSIS. Baptiste voulait le logo dans l'installateur et l'inventaire des options pertinentes. Ajouts dans `tauri.conf.json` : `installerIcon` et `uninstallerIcon`, un bandeau latéral (`sidebar.bmp`) et un en-tête (`header.bmp`) dessinés par `gen-images.ps1` à partir de l'icône de Cubee. Le sous-titre du bandeau est passé de « Le compagnon de Claude Code » à `for Claude Code`, puis `v0.3.0` a été ajouté et retiré : Baptiste a refusé toute étape PowerShell dans la release, les BMP sont donc commités et le script reste manuel ([BDR-095](decisions/BDR-095.md)).

Ensuite langues FR+EN avec sélecteur, page de licence AGPL et métadonnées (`publisher`, descriptions, `homepage`, `copyright`), avec des textes validés par Baptiste avant écriture ([BDR-096](decisions/BDR-096.md)). Le sélecteur de langue n'est jamais apparu à l'usage. J'ai d'abord regardé la clé de registre `baptistelechat`, puis un mini-installateur de test a prouvé que la config était correcte, et la lecture de `MANUFACTURER` dans le `installer.nsi` généré a montré que le changement de `publisher` avait déplacé la clé où NSIS mémorise la langue ([ZBLK-040](archive/blockers/ZBLK-040.md)). Commit `8904fd4`. Je n'ai pas pu vérifier moi-même le rendu des pages de licence ni le sélecteur, c'est Baptiste qui teste.

**Entrées clés :**

- [BDR-095](decisions/BDR-095.md) — bandeaux commités, script manuel hors CI
- [ZBLK-040](archive/blockers/ZBLK-040.md) — sélecteur de langue invisible : la clé de registre suit le publisher
- [LRN-110](learnings/LRN-110.md) — la langue NSIS est mémorisée par publisher

---

Sons du pet, de bout en bout, à partir de deux libs que Baptiste avait trouvées (uisfx, 12 feels, et Cuelume, un seul pack). J'ai recommandé uisfx pour la variété et la licence CC0 ([BDR-098](decisions/BDR-098.md)), puis construit avec lui la table événement → cue : ma première version (un cue par hook) forçait la diversité, il a demandé du réemploi, d'où 13 cues par émotion ([BDR-097](decisions/BDR-097.md)). Ajouts successifs : `wake` et `sleep` joués sur les transitions d'état (et `start` pour `SessionStart`), un mode « moins bavard » qui inverse l'ancien opt-in « Sons des outils » ([BDR-099](decisions/BDR-099.md)), une boucle d'ambiance `streaming` pendant que Claude travaille, deux curseurs de volume façon jeu avec l'ambiance à 30 % ([BDR-100](decisions/BDR-100.md), [BDR-101](decisions/BDR-101.md)), et un accordéon pour les réglages devenus longs ([BDR-103](decisions/BDR-103.md)). Le chantier a aussi ajouté un select avec les couleurs de marque des feels.

Trois retours d'écoute ont changé la direction. « On entend quasiment rien » : je n'ai pas cherché un bug de lecture, j'ai mesuré les 180 fichiers et trouvé 9 dB d'écart entre feels, résolu par une égalisation par pack ([LRN-114](learnings/LRN-114.md)). « Certains sons ne se lancent pas » : c'étaient les sons d'outils, coupés par défaut, que le nouvel aperçu de l'onglet Animation lève désormais. Puis le vrai blocage : la boucle jouée en double et qui ne s'arrêtait jamais ([BLK-041](blockers/BLK-041.md)). J'ai écarté le moteur avec un harnais Vite SSR à faux `AudioContext` ([LRN-117](learnings/LRN-117.md)), Baptiste a trouvé que l'aperçu du curseur, dans la fenêtre Réglages, jouait une 2e boucle par-dessus ([LRN-116](learnings/LRN-116.md)), et j'ai déduit que `AskUserQuestion` laissait le pet en `working`, un état sans timeout ([BDR-102](decisions/BDR-102.md), [LRN-112](learnings/LRN-112.md)). Cette dernière cause n'est pas confirmée à l'usage, le blocker reste ouvert. Le rituel de fin a produit deux commits : `980a4cd` (poussé) et `2a3d2b1` (correctif `AskUserQuestion`, non poussé). Baptiste a demandé de tout garder en local, sans entrée globale.

**Entrées clés :**

- [BDR-097](decisions/BDR-097.md) — un cue par émotion, réemploi assumé entre hooks
